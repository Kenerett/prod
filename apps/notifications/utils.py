...
def send_user_credentials_email(user, password=None, force_send_password=False):
    ...
    try:
        ...
        logger.error(
            f"Failed to send credentials email to {user.email} for user {user.username}: {str(e)}",
            exc_info=True
        )
        return False
...
def send_password_reset_email(user, new_password):
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Failed to render password reset email templates for user {user.username} ({user.email}): {str(tmpl_e)}"
            )
            plain_message = f"""
            Hello, {user.get_full_name() or user.username}!
        ...
        logger.error(
            f"Failed to send password reset email to {user.email} for user {user.username}: {str(e)}",
            exc_info=True
        )
        return False
